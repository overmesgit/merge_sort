...
def merge_sort(input_array, reverse=False):
    ...
    # if input_len == 2:
    #     if operator(input_array[1], input_array[0]):
    #         input_array.reverse()
    #     return input_array
    ...
    if input_len > 1:
        ...
        for k in range(input_len):
            if r == len(right_part) or l != len(left_part) and operator(left_part[l], right_part[r]):
    ...
    while merge_length < input_length:
        for l in range(0, input_length, merge_length*2):
            ...
            for k in range(l, r_last):
                if r == r_last or l != l_last and operator(input_array[l], input_array[r]):
